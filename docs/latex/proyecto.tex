...
\begin{figure}[H]
\centering
\includegraphics[height=0.82\textwidth]{../uml/png/u2a-estrategias.png}
\caption{Jerarquía de estrategias de recomendación.}
\label{fig:u2a}
\end{figure}
\end{landscape}
...
\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{../uml/png/u2b-pronostico-analitica.png}
\caption{Modelos de pronóstico y clasificadores de analítica.}
\label{fig:u2b}
\end{figure}

\subsection{El lift, que es lo que motivó todo}
Sobre $N$ pedidos, para dos productos $A$ y $B$:
\[
\mathrm{sop}(B)=\frac{n(B)}{N}, \qquad
\mathrm{conf}(A \to B)=\frac{n(A \cap B)}{n(A)}, \qquad
\mathrm{lift}(A \to B)=\frac{\mathrm{conf}(A \to B)}{\mathrm{sop}(B)}
\]

El resolutor anterior ordenaba por $n(A \cap B)$ a secas. Con los datos de
prueba, la gyoza y el refresco acompañan al ramen exactamente 50 veces cada uno,
así que ese criterio los deja empatados. Pero el refresco aparece en el 85\% de
todos los pedidos y la gyoza en el 38\%. Al dividir entre el soporte, el lift da
2.00 para la gyoza y 0.99 para el refresco: la gyoza sí acompaña al ramen, el
refresco nada más lo piden todos.

\subsection{Suavizado para los restaurantes chicos}
Un umbral duro (``ignorar los productos con menos de 20 pedidos'') deja sin
sugerencias justo a quien más las necesita, y además produce un salto brusco
entre 19 y 20 pedidos. En su lugar, la confianza se estima con un estimador $m$:
\[
\mathrm{conf}_\alpha(A \to B) = \frac{n(A \cap B) + \alpha \cdot \mathrm{sop}(B)}{n(A) + \alpha}
\]
con $\alpha \approx 10$ pedidos. Cuando hay poca evidencia sobre $A$, la
estimación se apoya en la popularidad general de $B$; conforme $n(A)$ crece, el
término $\alpha$ pierde peso y el valor converge a la confianza real.

El caso extremo sale bien solo: si $n(A) = 0$, la expresión se reduce a
$\mathrm{sop}(B)$ y por lo tanto $\mathrm{lift} = 1$, que es la respuesta honesta
cuando no hay información.

\subsection{Cascada de respaldo}
Aun con suavizado hay restaurantes sin ningún par medible. La cascada baja de
nivel hasta encontrar algo, y siempre encuentra:

\begin{longtable}{@{}cp{6.0cm}p{6.0cm}@{}}
\toprule
\textbf{Nivel} & \textbf{De dónde salen} & \textbf{Cuándo aplica} \\
\midrule
\endhead
1 & Co-ocurrencia de la sucursal & Hay señal suficiente \\
2 & Co-ocurrencia del restaurante & La sucursal sola no alcanza \\
3 & Co-ocurrencia con ventana de 365 días & Restaurante de poco volumen \\
...
7 & Productos destacados & Restaurante sin un solo pedido \\
\bottomrule
\end{longtable}

El nivel 4 es el que casi siempre resuelve, porque no necesita historial: si el
cliente lleva un plato fuerte y no lleva bebida, se le ofrece una bebida. Es la
regla que aplica cualquier mesero, y funciona desde el primer día.

Las categorías en cEats son texto libre, así que el tipo de cada una se deduce
en tres pasos: lo que el superadmin haya confirmado en el CRM, luego el precio
relativo a la mediana del menú, y por último un diccionario de palabras. Nada de
esto es aprendizaje automático, y es deliberado: con cuarenta pedidos una regla
le gana a un modelo entrenado, y además hay que poder explicarle al restaurantero
por qué se le sugiere lo que se le sugiere.

\subsection{Un error que salió corriendo la demo}
Con el ramen en el carrito, el sistema sugería refresco por afinidad de
categoría en lugar de la gyoza, que tenía lift 2.00. La causa: la afinidad
puntúa 3.0 y el lift 2.0, y comparados directamente gana el 3. Los puntajes de
escalones distintos no miden lo mismo y no se pueden comparar entre sí.

Cada escalón recibió una banda de cien puntos, de modo que el de arriba siempre
gana al de abajo y dentro de la banda el puntaje original sigue ordenando. Es un
error que ninguna prueba unitaria habría encontrado, porque cada pieza estaba
bien por separado.
...
\subsection{Igualdad de términos}
Dos términos alfa-equivalentes son el mismo término, así que la igualdad no
puede compararse por su escritura: $\lambda x.x$ y $\lambda y.y$ se escriben
distinto y significan lo mismo. La comparación usa índices de De Bruijn: cada
variable ligada se representa por la distancia al lambda que la liga, de modo
que el nombre desaparece y sólo queda la estructura. Las variables libres
conservan su nombre, porque ésas sí forman parte del significado.

Esto salió de una prueba en rojo, no del diseño: la prueba de conversión alfa
fallaba y tenía razón.
...
\begin{figure}[H]
\centering
\includegraphics[height=0.80\textwidth]{../uml/png/u4-patrones-y-capas.png}
